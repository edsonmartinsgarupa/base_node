...
export default class ProductRepository implements IProductRepository {
  ...
  public async deleteProducts(category: string): Promise<boolean> {
    const productsToDeleted = await this.#instanceDB.remove(
      await this.#instanceDB.find(ProductEntity, { where: { category } })
    );
    const notDeleted = productsToDeleted.some(
      (product) => product.uid !== undefined
    );
    return !notDeleted;
  }
}
